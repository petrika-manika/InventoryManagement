...
/// <summary>
/// Validator for CreateAromaDeviceCommand.
/// Ensures all product data meets business rules and constraints.
/// </summary>
public sealed class CreateAromaDeviceCommandValidator : AbstractValidator<CreateAromaDeviceCommand>
{
    public CreateAromaDeviceCommandValidator()
    {
        RuleFor(x => x.Name)
        ...
        RuleFor(x => x.SquareMeter)
            .GreaterThanOrEqualTo(0).WithMessage("Square meter must be greater than or equal to zero.")
            .When(x => x.SquareMeter.HasValue);
    }
