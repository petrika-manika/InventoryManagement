...
/// <summary>
/// Handler for UpdateAromaBombelCommand.
/// Updates an existing Aroma Bombel product in the database.
/// </summary>
public sealed class UpdateAromaBombelCommandHandler : IRequestHandler<UpdateAromaBombelCommand, Unit>
{
    private readonly IApplicationDbContext _context;

    /// <summary>
    /// Initializes a new instance of the <see cref="UpdateAromaBombelCommandHandler"/> class.
    /// </summary>
    /// <param name="context">The application database context.</param>
    public UpdateAromaBombelCommandHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    /// <summary>
    /// Handles the UpdateAromaBombelCommand request.
    /// </summary>
    ...
    public async Task<Unit> Handle(UpdateAromaBombelCommand request, CancellationToken cancellationToken)
    {
        // Find the product
        var product = await _context.Products
            .OfType<AromaBombelProduct>()
        ...
        if (isDuplicate)
        {
            throw new DuplicateProductNameException(request.Name, ProductType.AromaBombel);
        }
        ...
        // Convert taste ID to enum and update specific information
        TasteType? taste = request.TasteId.HasValue
            ? (TasteType)request.TasteId.Value
            : null;

        product.UpdateSpecificInfo(taste);
